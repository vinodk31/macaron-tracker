...
- **PINs are issued and checked by the server**, which is the only place that can tell whether a number is already in use at another location. A PIN can be the random one the server hands out, one the owner types in Setup, or one the employee picks for themselves on their **Account** tab — every route goes through the same uniqueness check. Changing your own PIN needs the current one, since a kiosk tablet is shared, and repeated failures lock out on their own counter rather than the login one. Owners keep seeing every PIN in Setup, whoever chose it. If a PIN ever did match two locations, login refuses rather than guessing.
...
- **Setup** — shop name, weekly store hours, prep/close buffers, staff & rates with sign-in PINs (**Set** to choose one, **Random** to issue one), products with their price, cost and flavors, and a two-step data wipe.
