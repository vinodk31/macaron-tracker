// Domain logic: defaults, the freezer-count → taken/restocked derivation,
...
// One definition of what a PIN looks like, used by login, by the staff PIN
// change and by the owner setting one by hand.
...
export function defaultSettings() {
  return {
    ...
    // A new shop starts on full 36-piece trays, which is the common case; a
    // shop that already has counts logged keeps counting them singly until
    // its owner sets a tray size, so nothing already entered changes meaning.
    flavors: [
      { id: makeId("flavor"), productId: MACARON_PRODUCT_ID, name: "Vanilla Bean", perTray: 36 },
      { id: makeId("flavor"), productId: MACARON_PRODUCT_ID, name: "Chocolate", perTray: 36 },
      { id: makeId("flavor"), productId: MACARON_PRODUCT_ID, name: "Raspberry", perTray: 36 },
      { id: makeId("flavor"), productId: MACARON_PRODUCT_ID, name: "Pistachio", perTray: 36 },
      { id: makeId("flavor"), productId: MACARON_PRODUCT_ID, name: "Salted Caramel", perTray: 36 },
    ],
  };
}
...
function migrateProducts(settings, rawSettings) {
  ...
  // Flavors saved before trays existed are counted singly until the owner
  // says otherwise, so they read back exactly as they were entered.
  const flavors = (settings.flavors || []).map((f) => ({
    ...f,
    productId: known.has(f.productId) ? f.productId : fallbackId,
    perTray: trayCapacity(f),
  }));
...
// ---------------------------------------------------------------------------
//
// Macarons come out of the freezer by the tray, and a tray holds 36 or 24
// depending on the flavor. Counts are still *stored* in single pieces, which
// is the unit everything is priced and sold in -- the tray size only changes
// how the number is typed in and read back. That also means changing a tray
// size never rewrites history: every count already logged keeps meaning
// exactly what it meant.

export function trayCapacity(flavor) {
  const n = Number(flavor?.perTray);
  return Number.isFinite(n) && n >= 1 ? Math.floor(n) : 1;
}

// A flavor counted singly has a capacity of 1, and the UI treats it as before.
export function isTrayed(flavor) {
  return trayCapacity(flavor) > 1;
}

export function unitsToTrays(units, flavor) {
  return units / trayCapacity(flavor);
}

export function traysToUnits(trays, flavor) {
  return Math.round(trays * trayCapacity(flavor));
}

// Whole trays print as "4", a part tray as "4.5". Two decimals is enough to
// show a third of a tray without turning the column into noise.
export function formatTrays(trays) {
  return String(Math.round(trays * 100) / 100);
}

// ---------------------------------------------------------------------------
// Freezer counts -> derived taken out / restocked
// ---------------------------------------------------------------------------
//
// A drop in the count means stock left the freezer, which is not the same
// thing as stock sold: a tray goes into the case and is sold over the hours
// that follow. The app can only see the freezer, so that is what it reports --
// what was taken out. Over a week the two converge, which is what makes it
// usable as a sales figure, but the wording never claims more than it knows.
// ---------------------------------------------------------------------------
...
// Day-over-day deltas for one flavor: a decrease was taken out, an increase
// was put back in. Days with no recorded count are skipped entirely, so the
// delta always compares against the last time the flavor was counted.
...
export function flavorTakenInRange(days, flavorId, startKeyInclusive, endKeyInclusive) {
  ...
  for (const f of flavors) {
    ...
    for (const d of deltas) {
      if (d.date >= startKey && d.date <= endKey) {
        if (!totals[d.date]) totals[d.date] = { date: d.date, taken: 0, restocked: 0 };
        totals[d.date].taken += d.taken;
        totals[d.date].restocked += d.restocked;
      }
    }
  }
  return eachDayInRange(startKey, endKey).map(
    (date) => totals[date] || { date, taken: 0, restocked: 0 }
  );
}
...
export function flavorRanking(days, flavors, startKey, endKey) {
  return flavors
    .map((f) => ({ flavor: f, ...flavorTakenInRange(days, f.id, startKey, endKey) }))
...
export function productBreakdown(settings, days, startKey, endKey) {
  return settings.products.map((product) => {
    const units = flavorsForProduct(settings, product.id).reduce(
      (sum, f) => sum + flavorTakenInRange(days, f.id, startKey, endKey).taken,
      0
    );
    return {
      product,
...
// the ingredient cost of everything that left the freezer in it, priced per
// product. Revenue prices what went out; wages are what was actually paid, so a
...
export function monthlyCogs(settings, days, payments, startKey, endKey) {
  ...
  const unitsTaken = byProduct.reduce((sum, row) => sum + row.units, 0);
  ...
  return {
    ...
    perMacaron: unitsTaken > 0 ? total / unitsTaken : null,
  };
}
