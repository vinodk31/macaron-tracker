...
// Returns { state, role, tenantId }. `state` is null when there is nothing
// stored yet; `role` says how much of the app the signed-in person may use;
// `tenantId` is the location they are working in, which is null for a
// franchisor who hasn't opened one.
export async function load() {
  const res = await fetch("/api/state", { cache: "no-store" });
  if (res.status === 401) throw new UnauthorizedError();
  if (!res.ok) throw new Error(`Could not load data (${res.status})`);

  const { data, role, tenantId } = await res.json();
  if (data) return { state: data, role, tenantId };
  ...
  if (legacy) {
    await save(legacy);
    return { state: legacy, role, tenantId };
  }
  return { state: null, role, tenantId };
}
